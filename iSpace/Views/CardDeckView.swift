...
struct CardDeckView: View {
    ...
    var body: some View {
        // UPDATED: Replaced the ScrollView/Overlay with a ZStack
        // to ensure the layout is correct when the view is empty.
        ZStack {
            if items.isEmpty {
                EmptyStateView(
...
// This helper view loads the necessary details for the CreditCardView to render.
    ...
    var body: some View {
        Group {
            if let details = cardDetails {
                CreditCardView(details: details)
            } else {
                // Show a placeholder with the correct aspect ratio while loading
